...
class Person extends \Orm\Model {
    ...
    public static function validate($factory) {
        ...
        $val->add_field('display, Display', 'max_length[162]');
...
//    ];
    ...
    public static function menuPersons() {
        $query = \DB::select('display', 'id')->from('persons')->order_by('display', 'desc')->as_assoc()->execute();
        $seasons = [];
        foreach ($query as $result) {
            $seasons[$result['display']] = $result['display'] . " " . $result['id'];
        }
